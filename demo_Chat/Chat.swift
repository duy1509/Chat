...
class Chat: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate,  UINavigationControllerDelegate, UIImagePickerControllerDelegate, CLLocationManagerDelegate {
    ...
    //Downloads messages
    func fetchData() {
        Message.downloadAllMessages(forUserID: uidMessage!, completion: {[weak weakSelf = self] (message) in
        ...
        Message.markMessagesRead(forUserID: auth.currentUser!.uid)
    }
    ...
    func composeMessage(type: MessageType, content: Any)  {
        ...
        Message.send(message: message, toID: uidMessage!, completion: {(_) in
        })
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .receiver:
            let cell = tableView.dequeueReusableCell(withIdentifier: "Receiver", for: indexPath) as! Receiver
            cell.clearCellData()
        ...
        case .sender:
            ...
            cell.clearCellData()
//            cell.profilePic.image = self.currentUser?.imageProfile
            switch self.items[indexPath.row].type {
            case .text:
                cell.message.text = self.items[indexPath.row].content as! String
            case .photo:
